...
# def get_dataloader(
#     tokenizer: PreTrainedTokenizer, rank: int, world_size: int, config: Config
# ) -> StatefulDataLoader:
#     datasets = [
#         create_dataset(
#             path=dataset.path,
...
#             trainer.train_step(step=step, batch=batch)
...
#             if step % 1000 == 0:
#                 self.save_checkpoint(step=step, trainer, dataloader)

#         self.cleanup()

#     def save_checkpoint(self, step: int, trainer: BasicGPTTrainer, dataloader: StatefulDataLoader):
#         # only rank 0 should save the checkpoint
#         if self.rank != 0:
#             return

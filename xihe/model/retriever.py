...
# 这个文件肯定是没有办法直接commit的
# 除非我们让pre commit不检查这个东西
...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states, masks):
        ...
        if self.flash:
            if masks is None:
                ...
                attn_mask = self.attention_mask[
                    :bsz, :, :seq_len, :seq_len
                ] + masks.view(bsz, 1, seq_len, seq_len)
                output = nn.functional.scaled_dot_product_attention(
                    query, key, value, attn_mask=attn_mask
                )
        else:
            att = torch.matmul(query, key.transpose(2, 3)) / math.sqrt(self.head_dim)
            assert att.size() == (bsz, self.num_heads, seq_len, seq_len), (
                "Attention weights shape error"
            )
            if masks is None:
                ...
                att = (
                    att
                    + self.attention_mask[:bsz, :, :seq_len, :seq_len]
                    + masks.view(bsz, 1, seq_len, seq_len)
                )
        ...
        assert output.size() == (bsz, seq_len, self.num_heads, self.head_dim), (
            "Attention output shape error"
        )
...
class Retriever(nn.Module):
    def __init__(self, device, ptdtype, config, flash=True):
        ...
        position_ids = torch.arange(
            0, config.sequence_length, dtype=torch.long, device=device
        )
        position_ids = position_ids.unsqueeze(0).view(-1, config.sequence_length)
        attention_mask = make_causal_mask(
            config.batch_size, config.sequence_length, device, ptdtype
        )
        self.layers = nn.ModuleList(
            [
                DecoderLayer(config, attention_mask, position_ids, flash)
    ...
    @torch.no_grad()
    def chat(
        self, tokenizer, message, history, temperature=1.0, top_k=1, max_new_tokens=256
    ):
        ...
        for i in range(max_new_tokens):
            input_trunc = (
                input
                if input.shape[1] <= self.config.sequence_length
                else input[:, -self.config.sequence_length :]
            )
...
def retriever(device, ptdtype, config, pretrained=False, model_path=None, flash=True):
    ...
    if pretrained:
        ...
        model.load_state_dict(
            {k.replace(replacer, ""): v for k, v in state_dict.items()}
        )
    return model
